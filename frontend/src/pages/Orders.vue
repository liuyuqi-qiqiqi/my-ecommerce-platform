...
async function loadOrders() {
  ...
  try {
    ...
    orders.value = result.items; page.value = result.page; pageSize.value = result.pageSize; total.value = result.total;
  } catch (err) { error.value = getOrderErrorMessage(err); }
  finally { loading.value = false; }
}

async function onPageChange(nextPage: number) { page.value = nextPage; await loadOrders(); }
</script>
    ...
    <div class="page">
      ...
      <div v-else-if="!orders.length" class="state empty">
        <el-empty description="暂无订单">
          <RouterLink to="/"><el-button type="primary">去首页逛逛</el-button></RouterLink>
        </el-empty>
      </div>
...
<style scoped>
.page { max-width: 800px; margin: 0 auto; padding: 0 24px 48px; }
.page h1 { margin: 0 0 20px; font-size: 22px; }
.list { display: flex; flex-direction: column; gap: 12px; }
.state { text-align: center; padding: 48px; color: #909399; }
.empty { background: #fff; border-radius: 12px; padding: 32px; box-shadow: 0 1px 4px rgba(0, 0, 0, 0.08); }
